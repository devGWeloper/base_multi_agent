...
#### 2. `src/core/` — 모든 버전이 공유하는 인프라 레이어

`llm.py`, `logging.py`, `exceptions.py`는 LLM 클라이언트 관리, 구조화 로깅, 예외 계층처럼 **어떤 버전·도메인에서도 변하지 않는 인프라 코드**다.
이것들을 버전 폴더 안에 두면, 버전이 늘어날수록 동일 코드를 반복 복사하게 된다.
`src/core/`에 한 번만 두고 모든 버전이 `from core.xxx import ...`로 공유한다.
...
의존 방향은 항상 **단방향 하향**. `core/`가 특정 버전 코드를 참조하거나, 버전 간 코드가 서로 참조하는 것은 금지.
...
이를 위해 두 경로가 Python path에 등록되어 있어야 한다.
...
| 실행 방식 | 경로 등록 방법 |
|----------|--------------|
| `pytest` | `pyproject.toml`의 `pythonpath = ["src/workflows/v1_0", "src"]` |
| `app.py` | 파일 상단 `sys.path.insert`로 두 경로 자동 등록 |
...
| 관심사 | 위치 | 내용 |
|--------|------|------|
| **로깅** | `src/core/logging.py` | `@log_node_execution` 데코레이터를 노드 함수에 적용하면 진입/종료, 소요시간, `intent`, `session_id`가 structlog에 자동 기록 |
